...
class TabModel(BaseEstimator):
    ...
    clip_value: int = 1
    verbose: int = 1
    optimizer_fn: Any = paddle.optimizer.Adam
    optimizer_params: Dict = field(default_factory=lambda: dict(lr=0.02))
    ...
    def __post_init__(self):
        ...
        self.device = str(define_device(self.device_name)).replace("cuda", "gpu")
        if self.verbose != 0:
    ...
    def fit(
        ...
        self.update_fit_params(X_train, y_train, eval_set, weights)
    ...
    def load_weights_from_unsupervised(self, unsupervised_model):
        ...
        self.network.set_state_dict(state_dict=update_state_dict)
    ...
    def save_model(self, path):
        ...
        paddle.save(
            obj=self.network.state_dict(), path=Path(path).joinpath("network.pt")
        )
        ...
        try:
            with zipfile.ZipFile(filepath) as z:
                ...
                with z.open("network.pt") as f:
                    try:
                        saved_state_dict = paddle.load(path=f)
                    except io.UnsupportedOperation:
                        saved_state_dict = paddle.load(path=io.BytesIO(f.read()))
        except KeyError:
            raise KeyError("Your zip file is missing at least one component")
        self.__init__(**loaded_params["init_params"])
        self._set_network()
        self.network.set_state_dict(state_dict=saved_state_dict)
        self.network.eval()
    ...
    def _train_batch(self, X, y):
        ...
        if self.clip_value:
            paddle.nn.utils.clip_grad_norm_(
                parameters=self.network.parameters(), max_norm=self.clip_value
            )
