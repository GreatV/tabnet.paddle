...
def UnsupervisedLoss(y_pred, embedded_x, obf_vars, eps=1e-09):
    """
    ...
    reconstruction_errors = paddle.multiply(x=errors, y=paddle.to_tensor(obf_vars)) ** 2
    batch_means = paddle.mean(x=embedded_x, axis=0)
    batch_means[batch_means == 0] = 1
    batch_stds = paddle.std(x=embedded_x, axis=0) ** 2
    batch_stds[batch_stds == 0] = batch_means[batch_stds == 0]
    features_loss = paddle.matmul(x=reconstruction_errors, y=1 / batch_stds)
    nb_reconstructed_variables = paddle.sum(x=obf_vars, axis=1)
    features_loss = features_loss / (nb_reconstructed_variables + eps)
    loss = paddle.mean(x=features_loss)
    return loss


def UnsupervisedLossNumpy(y_pred, embedded_x, obf_vars, eps=1e-09):
    errors = y_pred - embedded_x
