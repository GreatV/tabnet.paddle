...
class TorchDataset(paddle.io.Dataset):
    """
...
class SparseTorchDataset(paddle.io.Dataset):
    """
    ...
    def __getitem__(self, index):
        x = paddle.to_tensor(data=self.x[index].toarray()[0]).astype(dtype="float32")
        y = self.y[index]
        return x, y
...
class SparsePredictDataset(paddle.io.Dataset):
    """
    Format for csr_matrix
...
def create_sampler(weights, y_train):
    ...
    if isinstance(weights, int):
        if weights == 0:
            ...
            samples_weight = paddle.to_tensor(data=samples_weight)
            samples_weight = samples_weight.astype(dtype="float64")
...
def create_group_matrix(list_groups, input_dim):
    ...
    if len(list_groups) == 0:
        group_matrix = paddle.eye(num_rows=input_dim)
        return group_matrix
    else:
        n_groups = input_dim - int(np.sum([(len(gp) - 1) for gp in list_groups]))
        group_matrix = paddle.zeros(shape=(n_groups, input_dim))
...
def define_device(device_name):
    ...
    if device_name == "auto":
        if paddle.device.cuda.device_count() >= 1:
            ...
            return "cpu"
    elif device_name == "cuda" and not paddle.device.cuda.device_count() >= 1:
        return "cpu"
    else:
        return device_name
